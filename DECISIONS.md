...
## 19. BDD demo with playwright-bdd, not a full suite rewrite

`playwright-bdd` was added to demonstrate Cucumber/Gherkin proficiency alongside the existing Playwright suite, not to replace it. Only one representative flow (product search) was translated into `.feature` + step definitions — this is intentional scope, not an unfinished migration.

Two practical constraints shaped the implementation:

- `createBdd()` requires a `test` instance extended from `playwright-bdd` itself, not from `@playwright/test` (or our own `fixtures/index.ts`, which is built on top of it). Step definitions therefore construct `ProductListPage` manually from the built-in `page` fixture, rather than reusing the project's custom `productListPage` fixture.
